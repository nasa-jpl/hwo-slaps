...
def _load_observing_reference(
    path: Path,
    scene_labels: list[str],
) -> tuple[dict, dict]:
    """Load the observation block and per-scene source patches."""
    reference = _load_yaml_mapping(path, "Campaign observing reference")
...
def _load_frozen_manifest(output_root: Path) -> dict:
    ...
    root = _load_yaml_mapping(frozen_path, "Frozen campaign manifest")
...
def harvest_campaign(output_root) -> Path:
    ...
    harvest = {
        ...
        "campaign_uuid": campaign["campaign_uuid"],
        "name": campaign["name"],
        "manifest_frozen_sha256": _file_sha256(
