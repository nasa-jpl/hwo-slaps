"""Publication-grade Fisher / Asimov detector integrated with AutoLens + HCIPy.

This module is the package-aware bridge between the statistical core in
:mod:`hwoslaps.modeling.fisher_publication_core` and the HWO-SLAPS forward
model built on PyAutoLens and HCIPy.

The detector keeps the public pipeline interface close to the existing Fisher
v1 implementation, but replaces the underlying statistic with the profiled
linear-Gaussian amplitude test used by Asimov / Fisher forecasts.
...
Caveats
-------
This is still a *local* detector.  It is publication-grade in the sense that
its statistical object is rigorous, explicit, and sweepable, but it remains a
surrogate for the full nonlinear profile likelihood and therefore should be
calibrated against sparse full fits before the final manuscript.
"""
...
from .fisher_publication_adapter import (
...
from .fisher_publication_core import ProfileLikelihoodWorkspace, Whitener
...
class PublicationFisherDetector:
    """Publication-grade Fisher / Asimov detector for HWO-SLAPS.
    ...
    def __init__(
        ...
        self.publication_config = deepcopy(self.fisher_config.get("publication", {}))
        ...
        self.compute_psf_mode_scan = bool(
            self.publication_config.get("compute_psf_mode_scan", False)
        )
        z_tolerance = self.publication_config.get("mode_scan_z_tolerance", 1.0)
        self.mode_scan_z_tolerance = None if z_tolerance is None else float(z_tolerance)

        self.prior_sigmas = deepcopy(self.publication_config.get("prior_sigmas", {}))
        self.psf_mode_steps = deepcopy(self.publication_config.get("psf_mode_steps", {}))
        self.psf_mode_prior_sigmas = deepcopy(
            self.publication_config.get("psf_mode_prior_sigmas", {})
        )
        if "psf_mode_selection" in self.publication_config:
            raise ValueError(
                "modeling.fisher.publication.psf_mode_selection is not supported; use modeling.fisher.publication.psf_basis"
            )
        self.psf_basis_config = deepcopy(self.publication_config.get("psf_basis"))
        self.fit_psf_mode_selection = deepcopy(
            self.publication_config.get("fit_psf_mode_selection")
        )
        self.scan_psf_mode_selection = deepcopy(
            self.publication_config.get("scan_psf_mode_selection")
        )
        ...
        covariance_path = self.publication_config.get("covariance_path")
        if covariance_path is not None:
            ...
            if not os.path.exists(covariance_path):
                raise ValueError(
                    f"modeling.fisher.publication.covariance_path does not exist: {covariance_path}"
                )
        ...
        self.instrument_psf_mode_specs = self._build_psf_mode_specs_from_selection(
            ...
            context="modeling.fisher.publication.psf_basis",
        )
    ...
    def _resolve_fit_psf_mode_specs(self) -> List[_PsfModeSpec]:
        ...
        if self.fit_psf_mode_selection is None:
            ...
            specs = self._build_psf_mode_specs_from_selection(
                ...
                context="modeling.fisher.publication.fit_psf_mode_selection",
            )
    ...
    def _resolve_scan_psf_mode_specs(self) -> List[_PsfModeSpec]:
        ...
        specs = self._build_psf_mode_specs_from_selection(
            ...
            context="modeling.fisher.publication.scan_psf_mode_selection",
        )
        self._validate_psf_subset(
            specs,
            context="modeling.fisher.publication.scan_psf_mode_selection",
    ...
    def _validate_psf_subset(
        ...
        *,
        ...
        if missing:
            raise ValueError(
                f"{context} contains modes outside modeling.fisher.publication.psf_basis: {missing}"
            )
    ...
    def _build_mask(self) -> np.ndarray:
        ...
        if self.mask_mode != "source_snr":
            raise ValueError(
                "modeling.fisher.publication.mask_mode must be 'source_snr' or 'all_pixels'."
            )
