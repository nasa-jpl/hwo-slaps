...
def _truncated_config(truncation):
    """Return a valid NFWTruncated config carrying one truncation block."""
    config = _base_config()
    config["lensing"]["subhalo"]["enabled"] = True
    config["lensing"]["subhalo"]["model"] = "NFWTruncated"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"mode": "scale_ratio", "tau": 2},
        {"mode": "explicit_arcsec", "radius_arcsec": 0.05},
    ],
)
def test_truncated_nfw_truncation_blocks_are_accepted(truncation):
    """Accept both truncation modes for a truncated NFW subhalo."""
    VALIDATION.validate_or_raise(_truncated_config(truncation))


def test_truncated_nfw_requires_a_truncation_block():
    """Reject a truncated NFW subhalo with no truncation block."""
    config = _truncated_config({"mode": "scale_ratio", "tau": 10.0})
    config["lensing"]["subhalo"].pop("truncation")

    with pytest.raises(ValueError, match="Missing required key 'truncation'"):
        VALIDATION.validate_or_raise(config)


def test_truncated_nfw_requires_a_concentration_block():
    """Reject a truncated NFW subhalo with no concentration block."""
    config = _truncated_config({"mode": "scale_ratio", "tau": 10.0})
    config["lensing"]["subhalo"].pop("concentration", None)
...
@pytest.mark.parametrize("model", ["PointMass", "SIS", "NFW"])
def test_truncation_block_is_rejected_for_untruncated_models(model):
    """Reject a truncation block on a model that is not truncated."""
    config = _truncated_config({"mode": "scale_ratio", "tau": 10.0})
...
@pytest.mark.parametrize("bad_tau", [0.0, -1.0, np.nan, np.inf, True, "10"])
def test_truncation_scale_ratio_rejects_invalid_tau(bad_tau):
    """Reject a tau that is not a finite positive number."""
    _assert_rejected(_truncated_config({"mode": "scale_ratio", "tau": bad_tau}))


@pytest.mark.parametrize("bad_radius", [0.0, -0.05, np.nan, np.inf, True, "0.05"])
def test_truncation_explicit_arcsec_rejects_invalid_radius(bad_radius):
    """Reject a truncation radius that is not a finite positive number."""
    _assert_rejected(
        _truncated_config({"mode": "explicit_arcsec", "radius_arcsec": bad_radius})
    )


@pytest.mark.parametrize("bad_mode", ["scale-ratio", "explicit", "", 10.0, None])
def test_truncation_rejects_unknown_mode(bad_mode):
    """Reject a truncation mode outside the supported set."""
    _assert_rejected(_truncated_config({"mode": bad_mode, "tau": 10.0}))


def test_truncation_requires_a_mode():
    """Reject a truncation block with no declared mode."""
    with pytest.raises(ValueError, match="Missing required key 'mode'"):
        VALIDATION.validate_or_raise(_truncated_config({"tau": 10.0}))
...
@pytest.mark.parametrize(
    ...
    [
        {"mode": "scale_ratio"},
        {"mode": "explicit_arcsec"},
        {"mode": "scale_ratio", "tau": 10.0, "radius_arcsec": 0.05},
        {"mode": "explicit_arcsec", "tau": 10.0, "radius_arcsec": 0.05},
        {"mode": "scale_ratio", "tau": 10.0, "tau_max": 20.0},
    ],
)
def test_truncation_rejects_missing_or_cross_mode_parameters(truncation):
    """Reject truncation keys the selected mode does not accept."""
    _assert_rejected(_truncated_config(truncation))


@pytest.mark.parametrize("model", ["NFW", "NFWTruncated"])
def test_explicit_concentration_model_is_accepted(model):
    """Accept a directly declared c200 for both NFW variants."""
    config = _truncated_config({"mode": "scale_ratio", "tau": 10.0})
    config["lensing"]["subhalo"]["model"] = model
    if model == "NFW":
        config["lensing"]["subhalo"].pop("truncation")
...
def test_explicit_concentration_requires_c200():
    """Reject the explicit concentration model with no c200 value."""
    config = _base_config()
    config["lensing"]["subhalo"]["enabled"] = True
    config["lensing"]["subhalo"]["model"] = "NFW"
    config["lensing"]["subhalo"]["concentration"] = {"model": "explicit"}

    with pytest.raises(ValueError, match="Missing required key 'c200'"):
        VALIDATION.validate_or_raise(config)


@pytest.mark.parametrize("bad_c200", [0.0, -1.0, np.nan, np.inf, True, "15"])
def test_explicit_concentration_rejects_non_positive_c200(bad_c200):
    """Reject an explicit c200 that is not a finite positive number."""
    config = _base_config()
    config["lensing"]["subhalo"]["enabled"] = True
    config["lensing"]["subhalo"]["model"] = "NFW"
    config["lensing"]["subhalo"]["concentration"] = {
...
@pytest.mark.parametrize(
    "concentration",
    [
        {"model": "explicit", "c200": 15.0, "x_sub": 1.0},
        {"model": "explicit", "c200": 15.0, "h": 0.6774},
    ],
)
def test_explicit_concentration_rejects_relation_inputs(concentration):
    """Reject relation inputs supplied to the explicit concentration model."""
    config = _base_config()
    config["lensing"]["subhalo"]["enabled"] = True
    config["lensing"]["subhalo"]["model"] = "NFW"
    config["lensing"]["subhalo"]["concentration"] = concentration
...
@pytest.mark.parametrize(
    "concentration",
    [
        {"model": "moline2017_eq7", "x_sub": 1.0, "h": 0.6774, "c200": 15.0},
        {"model": "power_law", "c200": 15.0},
    ],
)
def test_c200_is_rejected_for_relation_concentration_models(concentration):
    """Reject a declared c200 for models that derive it from a relation."""
    config = _base_config()
    config["lensing"]["subhalo"]["enabled"] = True
    config["lensing"]["subhalo"]["model"] = "NFW"
    config["lensing"]["subhalo"]["concentration"] = concentration

    with pytest.raises(ValueError, match="c200 is supported only when"):
        VALIDATION.validate_or_raise(config)
...
@pytest.mark.parametrize(
    "concentration",
    [
        {"model": "moline2017_eq7", "x_sub": 1.0, "h": 0.6774, "offset_dex": 0.0},
        {"model": "moline2017_eq7", "x_sub": 1.0, "h": 0.6774, "offset_dex": -0.5},
        {"model": "power_law", "offset_dex": 0.301},
        {"model": "explicit", "c200": 15.0, "offset_dex": 0.602},
        {"model": "power_law", "offset_dex": None},
    ],
)
def test_concentration_offset_dex_is_accepted_by_every_model(concentration):
    """Accept the optional log10 concentration offset for every model."""
    config = _base_config()
    config["lensing"]["subhalo"]["enabled"] = True
    config["lensing"]["subhalo"]["model"] = "NFW"
    config["lensing"]["subhalo"]["concentration"] = concentration
...
@pytest.mark.parametrize("bad_offset", [np.nan, np.inf, -np.inf, True, "0.5"])
def test_concentration_offset_dex_rejects_non_numeric_or_non_finite_values(bad_offset):
    """Reject a concentration offset that is not a finite number."""
    config = _base_config()
    config["lensing"]["subhalo"]["enabled"] = True
    config["lensing"]["subhalo"]["model"] = "NFW"
    config["lensing"]["subhalo"]["concentration"] = {
        "model": "power_law",
        "offset_dex": bad_offset,
    }

    with pytest.raises(ValueError, match="offset_dex must be"):
        VALIDATION.validate_or_raise(config)
